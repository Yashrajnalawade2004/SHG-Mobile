...
let c = fs.readFileSync('scratch/patch-internal-loan.js', 'utf8');

// 1. TIMELINE REDESIGN
const timelineStart = c.indexOf('<View style={styles.timelineCard}>');
const timelineEnd = c.indexOf('</View>', c.indexOf('</View>', c.indexOf('</View>', c.indexOf('</View>', timelineStart) + 1) + 1) + 1) + 7; 
// Wait, simple replacement:
c = c.replace(/<View style=\{styles\.timelineCard\}>[\s\S]*?(?=\{showTreasurerActions &&)/, `
        {/* NEW LIFECYCLE TIMELINE */}
        <View style={styles.timelineCard}>
          <Text style={styles.timelineTitle}>{t("loan_lifecycle")}</Text>
          ...
          {(loan.status === "completed" || loan.status === "approved") && (
            <View style={[styles.timelineStep, { opacity: loan.status === "completed" ? 1 : 0.4 }]}>
              ...
              <Text style={styles.timelineMeta}>
                {loan.status === "completed" ? t("completed") : t("pending")}
              </Text>
            </View>
          )}
            ...
            <View style={[styles.timelineStep, { opacity: loan.status === "approved" || loan.status === "completed" ? 1 : 0.4 }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                <Text style={[styles.timelineStepTitle, loan.status === "approved" && { color: Colors.light.primary }]}>
                  {t("monthly_repayments")} {loan.status === "approved" ? "(In Progress)" : ""}
                </Text>
              </View>
              <Text style={styles.timelineMeta}>
                {loan.status === "completed" ? t("completed") : loan.status === "approved" ? t("active") : t("pending")}
              </Text>
            </View>
          )}
          ...
          {(loan.status === "completed" || loan.status === "approved") && (
            <View style={styles.timelineStep}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Ionicons name="checkmark-circle" size={16} color={Colors.light.success} />
                <Text style={styles.timelineStepTitle}>{t("loan_disbursed")}</Text>
              </View>
              <Text style={styles.timelineMeta}>
                {loan.approvedAt ? new Date(loan.approvedAt).toLocaleDateString("en-IN") : t("completed")}
              </Text>
            </View>
          )}
            ...
            <View style={styles.timelineStep}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                <Text style={[styles.timelineStepTitle, (loan.status === "rejected" || loan.status === "treasurer_rejected") && { color: Colors.light.danger }]}>
                  {(loan.status === "approved" || loan.status === "completed") ? t("president_approved") : t("loan_rejected")}
                </Text>
              </View>
              <Text style={styles.timelineMeta}>
                {loan.approvedAt ? new Date(loan.approvedAt).toLocaleDateString("en-IN") : loan.rejectedAt ? new Date(loan.rejectedAt).toLocaleDateString("en-IN") : t("completed")}
              </Text>
            </View>
          ) : (
            <View style={styles.timelineStep}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                <Text style={[styles.timelineStepTitle, { color: "#D97706" }]}>{t("president_review")} (Pending)</Text>
              </View>
...
// 2. PASSBOOK VISIBILITY
c = c.replace(/const showRepayment = loan\.status === "approved";/g, 'const showRepayment = loan.status === "approved" || loan.status === "completed";');

// 3. PASSBOOK LEDGER REDESIGN
// We need to replace the mapped displayEntries block with the new Passbook Table
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator>
                <View>
                  ...
                  <View style={[styles.tableRow, styles.tableHeader]}>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 90 }]}>{t("date")}</Text>
                  ...
                  {loan.status === "completed" && (
                    <View style={[styles.tableRow, { backgroundColor: Colors.light.primary + "20", borderTopWidth: 2, borderTopColor: Colors.light.primary }]}>
                       <Text style={[styles.tableCell, { width: isReducingBalance ? 90 + 110 + 120 + 120 + 110 : 90 + 120, fontFamily: "Poppins_700Bold" }]}>{t("bank_loan.total")}</Text>
                       {isReducingBalance && (
                         <>
            ...
            {loan.status === "completed" && (
              <View style={{ marginTop: 24, backgroundColor: Colors.light.success + "15", padding: 16, borderRadius: 12, borderWidth: 1, borderColor: Colors.light.success + "50" }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  <Text style={{ fontFamily: "Poppins_700Bold", fontSize: 16, color: Colors.light.success }}>Fully Repaid & Settled</Text>
                </View>
                <View style={{ gap: 8 }}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ fontFamily: "Poppins_500Medium", color: Colors.light.textSecondary }}>Loan Amount</Text><Text style={{ fontFamily: "Poppins_600SemiBold", color: Colors.light.text }}>Rs. {loan.amount.toLocaleString("en-IN")}</Text></View>
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ fontFamily: "Poppins_500Medium", color: Colors.light.textSecondary }}>Total Principal Paid</Text><Text style={{ fontFamily: "Poppins_600SemiBold", color: Colors.light.text }}>Rs. {(loan.totalPrincipalPaid || loan.amount).toLocaleString("en-IN")}</Text></View>
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ fontFamily: "Poppins_500Medium", color: Colors.light.textSecondary }}>Total Interest Paid</Text><Text style={{ fontFamily: "Poppins_600SemiBold", color: Colors.light.text }}>Rs. {(loan.totalInterestPaid || 0).toLocaleString("en-IN")}</Text></View>
                  <View style={{ borderTopWidth: 1, borderTopColor: Colors.light.border, marginVertical: 4 }} />
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ fontFamily: "Poppins_700Bold", color: Colors.light.text }}>Total Amount Repaid</Text><Text style={{ fontFamily: "Poppins_700Bold", color: Colors.light.primary }}>Rs. {isReducingBalance ? ((loan.totalPrincipalPaid || 0) + (loan.totalInterestPaid || 0)).toLocaleString("en-IN") : totalRepaid.toLocaleString("en-IN")}</Text></View>
                </View>
              </View>
            )}
`;
...
// Also need to add tableRow, tableCell, tableHeader to styles if they don't exist.
...
fs.writeFileSync('scratch/patch-internal-loan.js', c);
console.log('Done!');
